...
def generate_slug(title: str) -> str:
    """Генерирует slug из заголовка: транслитерация + замена спецсимволов на дефисы."""
    # Простая транслитерация (можно дополнить)
    translit_map = {
    ...
    return slug or 'post'  # если совсем пусто, запасной вариант
...
def handle_get(conn, event):
    ...
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        if slug:
            # Защита от инъекций через параметризацию
            cur.execute(
                f"""
...
def handle_put(conn, event):
    ...
    # Список обновляемых полей (расширьте при необходимости)
    updatable = {
        'title': 'title',
        'excerpt': 'excerpt',
        'content': 'content',
        ...
        'views': 'views',  # если нужно изменять вручную
    }
    ...
    if 'tags' in body:
        ...
        if isinstance(tags, list):
            ...
            set_clauses.append(f"tags = %s::text[]")
            params.append(tags)
        else:
    ...
    # Добавляем updated_at (если есть такое поле)
...
def handle_post(conn, event):
    ...
    # Все поля таблицы со значениями по умолчанию
    insert_data = {
        'slug': slug,
    ...
    # Формируем запрос INSERT с возвратом всех полей
    columns = []
    values_placeholders = []
    params = []
