...
export const FilterPanel = memo(
  ({
    ...
    return (
      <div
        className={`bg-card border border-primary/20 rounded-2xl shadow-lg mb-3 sm:mb-4 relative overflow-hidden transition-all duration-300 ease-in-out ${isExpanded ? "max-w-full" : "max-w-[190px] sm:max-w-[230px]"}`}
      >
        <button
          onClick={(e) => {
        ...
        {isExpanded && (
          <div
            className="space-y-5 sm:space-y-6 px-6 pb-6"
            onClick={(e) => e.stopPropagation()} // Предотвращаем клики внутри
          >
