        ...
        <>
          ...
          {filteredProviders.length > providersToShow && (
            <div className="flex flex-col sm:flex-row justify-center items-center gap-2 mt-8">
              ...
              {/* Кнопка "Показать всех провайдеров / Свернуть" */}
              <button
                onClick={() => {
                  if (providersToShow === filteredProviders.length) {
                    // Если все провайдеры показаны, возвращаемся к 9
                    const initialShow = Math.min(9, filteredProviders.length);
                    setProvidersToShow(initialShow);
                  } else {
              ...
              >
                ...
                <span className="relative flex items-center justify-center gap-2">
                  ...
                  <svg
                    className={`w-5 h-5 transition-transform ${providersToShow === filteredProviders.length ? "rotate-180" : "group-hover:translate-y-1"}`}
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
